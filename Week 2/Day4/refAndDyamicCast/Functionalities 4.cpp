...
void FindLowestSalary(const Container &data)
{
    ...
    for (const Pointer &ptr : data)
    {
       ...
       std::cout <<"Find lowest salary : "<< *Lowest;// display all attributes of FullTimeEmployee

   // std::cout <<"Find lowest salary : "<< *std::dynamic_pointer_cast<FullTimeEmployee>(Lowest);// display all attributes of FullTimeEmployee
}
...
std::string FindProjectName(const Container &data, std::string id)
{
    ...
    for (const Pointer &ptr : data)
    {
        if (ptr->id() == id)
        {
            return std::dynamic_pointer_cast<FullTimeEmployee>(ptr)->projectName();  // we use dynamic pointer cast to access the objects from parent and child class if we dont use dynamic pointer cast then it will print only parent class object 
        }
    }
